...
When a snapshot policy is assigned to more than one volume, you must manually remove it from all volumes to delete the snapshot policy. *How do you remove a snapshot policy?* Alternatively, you can assign a different snapshot policy to the volumes. 
