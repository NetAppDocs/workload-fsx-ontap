...
.Before you begin
To replicate a storage VM to another FSx for ONTAP file system, you must associate a link. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[Learn how to associate an existing link or to create and associate a new link^]. After the link associates, return to this operation. 
...
. From *FSx for ONTAP*, select the actions menu of the file system with the storage VM to replicate and then select *Manage*.
. In the file system overview under Storage VMs, select *Manage*.
. On the Manage storage VMs screen, select the actions menu of the storage VM to replicate an SVM for, then select *Advanced actions* > *Replicate storage VM*. 
