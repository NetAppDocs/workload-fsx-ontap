...
+
*Replicate storage VM configuration*: Optionally, select to replicate storage VM data and configuration settings for a single storage VM. When migrating data and settings, all volumes must finish replication in the last 24 hours and then cut over at the same time.
+
The tiering policy for all volumes defaults to the tiering policy of the source volume, which is recommended for migration use cases.
+
During migration, clients access data from the source ONTAP file system. After cutover, volumes are available through the target FSx for ONTAP file system IP addresses. Update any clients or resources that still use the old IP address.
...
.. *Storage VM name*: Select the storage VM. The storage VM you select is the target for all selected volumes in this replication relationship.
+
When the source and target storage VM names are identical, we create a file system alias for the target storage VM names to avoid peering conflicts.
...
.. *Initialize replication relationship*: Choose to initialize the replication relationship immediately after creating the replication relationship or later. Note that initialization can be time-consuming, and you might want to run the baseline transfer in off-peak hours.
+ 
To initialize later manually, refer to the instructions in <<Step 2: Initialize the replication relationship,Step 2: Initialize the replication relationship>>.
...
* *Choose an existing policy*: select an existing policy. 
...
.. *S3 access point tags*:  Optionally, you can add up to 50 tags.
