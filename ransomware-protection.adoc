...
* *NAS volumes (NFS/SMB)*: With ARP/AI, there is no learning period. ARP/AI is active immediately after you enable it. The system uses a pre-trained machine learning model to detect ransomware threats right away.

* *SAN volumes (beginning with ONTAP 9.17.1)*: ARP/AI provides immediate active protection even during an initial evaluation period. The evaluation period (2-4 weeks) establishes baseline encryption behavior and adaptive thresholds, while ARP/AI actively monitors and protects your data during this time. Detection and alerts can occur while baseline thresholds are being established.

* *Active mode*: In active mode, FSx for ONTAP creates ARP/AI snapshots to protect the data if a threat is detected. If a file extension is flagged as abnormal, you should evaluate the alert. You can act on the alert to protect your data or you can mark the alert as a false positive. Marking an alert as a false positive updates the alert profile. For example, if the alert is triggered by a new file extension and you mark the alert as a false positive, you will not receive an alert the next time that file extension is observed.
...
Enabling ARP/AI for a file system adds protection for all existing and newly created NAS (NFS/SMB) volumes automatically. Beginning with ONTAP 9.17.1, SAN volumes (iSCSI) are also supported. You can also enable ARP/AI for individual volumes.
