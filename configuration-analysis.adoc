...
* *Connect host to failover node*
+
Workload Factory recommends connecting hosts to the failover node to maintain connectivity and avoid disruptions during failovers.
...
* *Connect host to preferred node*
+ 
In a high-availability FSx for ONTAP deployment model, hosts should be connected directly to the node that serves the block device to ensure direct access and optimal performance. 
