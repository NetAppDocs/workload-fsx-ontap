...
* *EMS events*: Error, alert, and emergency events from the ONTAP Event Management System. To view and analyze EMS events, you must link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[connect to the file system using a link^].

* *Latency events*: Read and write latency events. Latency events can be viewed without a link connection. To analyze latency events and receive AI-powered insights, you must link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[connect to the file system using a link^].

To view events, you must link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html[grant _view, planning, and analysis_ permissions^] in your AWS account. Events are displayed for 72 hours before removal.
...
* Advanced troubleshooting: AI automatically identifies, analyzes, and provides insights to fix FSx for ONTAP EMS and latency events, reducing manual investigation time.
* Best-practice remediation: The event analyzer gives clear, actionable steps to resolve FSx for ONTAP EMS and latency events.
...
NOTE: If your account admin has turned off AI features, you'll see a message and won't be able to use AI analysis. Contact your administrator or https://docs.netapp.com/us-en/workload-setup-admin/manage-genai-features.html[learn about AI administration^].
...
To view latency events you must do the following: 

* link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html[grant _view, planning, and analysis_ permissions^] in your AWS account.

* link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[connect to the file system using a link^].

To analyze EMS and latency events and receive AI-powered insights and recommendations, ensure the following:

* You have link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html[_operations and remediation_ permissions^] in your AWS account. 
* You have link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[connected to the file system using a link^].
...
+
link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html^[Learn how to associate an existing link or to create and associate a new link]. After the link associates, return to this operation.
...
. From the Analysis screen, select the AWS accounts, credentials, and regions that contain the FSx for ONTAP file systems you want to view.
+
Only FSx for ONTAP file systems that have events are shown.
. Select the FSx for ONTAP file system you want to view.
+
The events table shows all events for that file system.
. Review the event details in the table.
. To analyze events, do one of the following:
+
* To analyze multiple events, select the checkboxes next to the events, then select *Analyze* in the table.
* To analyze one event and view insights in the side panel, select the event name, and then select *Analyze* in the panel. The insights and recommendations are shown in the panel.
* To analyze one event from the table, select *Analyze* in the event row, then select the event name to view the insights and recommendations.
+
...
* link:set-volume-latency-thresholds.html[Set volume latency thresholds in Workload Factory]