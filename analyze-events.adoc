...
---
= Analyze events for FSx for ONTAP file systems in Workload Factory
:icons: font
:imagesdir: ./media/

[.lead]
Use the smart event analyzer to automatically extract and analyze Event Management System (EMS) events for FSx for ONTAP file systems so that you can quickly identify and resolve issues. The Agentic AI-based analysis requires Amazon Bedrock integration.

== About this task 

Storage administrators often respond to EMS events for FSx for ONTAP only after customer complaints or by creating custom scripts and alarms. This reactive approach slows down fixing issues, increases downtime, and makes operations less efficient.

The event analyzer extracts EMS events for FSx for ONTAP and analyzes them to help you maintain the health and performance of FSx for ONTAP file systems. 
...
+
The event analyzer uses Amazon Bedrock APIs to provide AI-based insights for FSx for ONTAP events.
+
