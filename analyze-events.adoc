...
The event analyzer extracts FSx for ONTAP EMS events and analyzes them to help you maintain the health and performance of FSx for ONTAP file systems. Workload Factory displays only events that occurred in the last 72 hours.
...
+
The event analyzer uses Amazon Bedrock APIs to provide AI-based insights for FSx for ONTAP events. Configure Amazon Bedrock for each AWS account separately.
+
...
. Select the FSx for ONTAP file system to analyze and then select *Manage*.
