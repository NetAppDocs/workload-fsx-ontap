...
Storage administrators often respond to FSx for ONTAP EMS events only after customer complaints or by creating custom scripts and alarms. This reactive approach delays issue resolution, increases downtime, and reduces efficiency.

The event analyzer extracts the following FSx for ONTAP EMS events: error, warning, and emergency. You can view the events if you connect to the FSx for ONTAP file system with a link. 

With Amazon Bedrock integration, Workload Factory uses AI to analyze the events and provide insights to help you maintain the health and performance of FSx for ONTAP file system. Workload Factory displays error, warning, and emergency events. Events are removed after 72 hours.

Key benefits include:

* Advanced troubleshooting: Leverages AI to automatically find and analyze events, reducing the time and effort needed for manual investigation.
* Best-practice remediation: Gives clear steps to fix FSx for ONTAP EMS events so anyone can resolve issues.
...
== Before you begin
To use the event analyzer, you must meet the following requirements:
