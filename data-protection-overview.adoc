...
keywords: overview
summary: "FSx for ONTAP supports snapshots, NetApp Autonomous Ransomware with AI, volume replication, and backups in NetApp Workload Factory."
---
...
.Replication
Two types of volume replication are available for Amazon FSx for NetApp ONTAP: replication using ONTAP SnapMirror technology and ONTAP FlexCache technology.

Replication using ONTAP SnapMirror technology creates a secondary copy of your data on another FSx for ONTAP file system and continually updates the secondary data. Your data is kept current and remains available whenever you need it, such as for disaster recovery.

Replication using ONTAP FlexCache technology creates a writable cache volume that provides low-latency access to data stored in an origin, or source, volume. The cache can be located in a different, optionally remote, file system. Instead of copying all data from the origin volume, FlexCache copies data only as needed. FlexCache volumes are ideal for read-intensive workflows with infrequent data changes; any changes to the origin data require the cache to be refreshed.
