...
After you resolve the issue, you must submit the CloudTrail ARN again to restart the journal table initiation process.
...
.Resolution
Remove the `LogsToS3TableIntegration` and `S3TableIntegrationRole` resources from the template before deploying the CloudFormation stack again. The existing integration will be used automatically as long as it is configured for aws_cloudtrail data events.
...
After submitting the CloudTrail ARN, Workload Factory validates the entire pipeline by sending a _seed_, or test, event automatically. If successful, the seed event arrives in the S3 Tables table. The test takes approximately 10 minutes. 
...
| The S3TableIntegration never created the CloudWatch-managed table. Pipeline is broken between CloudWatch Logs and S3 Tables.

| Table exists, but the journal seed event did not appear. Verify CloudTrail and CloudWatch permissions.
...
After you resolve the issue — whether by fixing a missing permission, removing a conflicting resource from the template, or re-deploying the stack — return to the Workload Factory console, and retry <<Set up the journal table infrastructure,initiating the journal table setup>> by submitting the Trail ARN again.
