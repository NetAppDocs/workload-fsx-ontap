...
summary: "Discover and replicate on-premises ONTAP data in NetApp Workload Factory so it can be used as a backup in the cloud."
---
...
[.lead]
Discover and replicate on-premises ONTAP data in NetApp Workload Factory so it can be used as a backup in the cloud.
...
* A connected link to associate with the discovered on-premises cluster. If you don't have a link, you'll need to link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[create one^]. 
...
.. *ONTAP credentials*: Provide the following:
+
* GovCloud accounts: Enter ARN with ONTAP credentials.
* All other accounts: Enter the ONTAP credentials for the on-premises ONTAP cluster. Make sure the user has the required permissions. 
...
* Both the on-premises ONTAP cluster and the FSx for ONTAP file system you use for the replication relationship must have an associated link. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[Learn how to associate an existing link or to create and associate a new link^]. After the link associates, return to this operation. 
...
. Select the actions menu and select *Remove from Workload Factory*. 
