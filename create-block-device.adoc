...
[.lead]
Create block devices to support your line of business (LOB) application requirements. You can create block devices in an existing volume, create a new volume for the new block device, or create a new volume from a snapshot for the new block device. 
...
.Before you begin
* You must associate a link to create block devices. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[Learn how to associate an existing link or to create and associate a new link^]. After you associate the link, return to this operation.
...
* Create from a volume snapshot. Select the source volume name and snapshot name. Only volumes without block devices are available. 
.. *Aggregate name*: Select the aggregate to host the new volume.
...
link:https://docs.netapp.com/us-en/workload-fsx-ontap/manage-block-storage.html[Manage the block storage for an FSx for ONTAP file system]