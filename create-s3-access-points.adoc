...
Optionally, you can use the Journal table feature with S3 access points in Workload Factory. The journal table provides a log of access point events, which can be useful for monitoring and troubleshooting. To enable the journal table feature, you need to set up the necessary AWS infrastructure, including AWS CloudTrail, AWS CloudWatch, AWS S3 Buckets, AWS CloudWatch log groups, and AWS Identity and Access Management (IAM) roles and policies. For more details on setting up the journal table infrastructure, refer to link:setup-journal-table.adoc[Set up the journal table infrastructure for NetApp Workload Factory^].
