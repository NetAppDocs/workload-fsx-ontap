...
* *Retention modes*: You can select from two retention modes - _Enterprise_ or _Compliance_.
...
** To configure access to the volume, you must associate a link. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[Learn how to associate an existing link or to create and associate a new link^]. After the link associates, return to this operation. 
...
+
* A _FlexVol volume_ is a type of logical storage in ONTAP that allows you to manage data. FlexVol volumes can be moved between aggregates or nodes within the same storage virtual machine (SVM) without disrupting client access. The FlexVol volume style is selected by default. 
+
* A _FlexGroup_ volume is a scale-out NAS container that provides high performance along with automatic load distribution and scalability. It consists of several member volumes (constituents), which are underlying FlexVol volumes that share traffic transparently. 
...
.. *S3 access point*: Optionally, attach an S3 access point to access FSx for ONTAP file system data residing on NFS or SMB/CIFS volumes via AWS S3 APIs. Only the file access type is supported. Providing the following details: 
...
+
The following are default policies from AWS. To display existing snapshot policies, you must link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[associate a link^]. 
+
...
.. *Immutable files*: Enabling the immutable files feature permanently commits files in this volume to an immutable WORM (write-once-read-many) state. Provide the following details:
...
+
* A link is not associated with the file system. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html[Learn how to associate an existing link or to create and associate a new link^]. After the link associates, return to this operation.
* Volumes with immutable files and volumes with the NVMe protocol are not supported for ARP/AI. iSCSI volumes are supported beginning with ONTAP 9.17.1.
* The file system already has an ARP/AI policy.

. Under Advance configuration, enter the following details. The fields under this section differ based on the volume style you select. 
...
.. *Aggregate name*: Only for FlexVol volumes. Select the aggregate to host the new volume.
