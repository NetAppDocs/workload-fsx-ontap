...
.. *ARP/AI*: Optional. Select to enable NetApp Autonomous Ransomware Protection with AI (ARP/AI) for the volume and accept the statement to proceed. link:https://docs.netapp.com/us-en/workload-fsx-ontap/ransomware-protection.html[Learn more about ARP/AI].

A link must be associated with the file system to enable ARP/AI. link:https://docs.netapp.com/us-en/workload-fsx-ontap/create-link.html[Learn how to associate an existing link or to create and associate a new link]. After the link associates, return to this operation.
+
NOTE: Volumes with immutable files, and volumes with iSCSI and NVMe protocols are not supported for ARP/AI.
