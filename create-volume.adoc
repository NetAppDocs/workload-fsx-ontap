...
. To configure access to the volume, you must associate a link. To associate a link in the file system, click *Associate link* under *Account name*. Once the link associates, return to this operation. 
...
+
After volume creation, you can rebalance volume capacity but only for FlexGroup volumes. FlexGroup rebalancing helps redistribute capacity when imbalances develop over time due to the addition of new files and file growth. 
+
...
+
The following are default policies from AWS. For custom snapshot policies, you need to associate a link. 
+
