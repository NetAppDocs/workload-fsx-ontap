...
. From *FSx for ONTAP*, select the three-dot menu of the file system you want to create a volume in, and select *Create volume*. 
...
.. *S3 endpoint*: Optionally, attach an S3 endpoint to access FSx for ONTAP file system data residing on NFS or SMB/CIFS volumes via AWS S3 APIs. Only the file access type is supported. Providing the following details: 

* *S3 endpoint name*: Enter the name of the S3 endpoint.
