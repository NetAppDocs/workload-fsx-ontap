...
== Cutover replication for storage VM migration use cases

Cut over replication for storage VM migration use cases isn't supported for volumes with S3 access points or ONTAP S3 configurations including object-store-server, buckets, users, groups, or policies on the vserver.
