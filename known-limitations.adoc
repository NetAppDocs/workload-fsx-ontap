...
---
= Known limitations of FSx for ONTAP in NetApp Workload Factory
:icons: font
:imagesdir: ./media/
...
* Workload Factory doesn't support migration replication for storage VMs between first and second generation FSx for ONTAP file systems.

* Cut over replication isn't supported for volumes with S3 access points or ONTAP S3 configurations including object-store-server, buckets, users, groups, or policies on the vserver.
