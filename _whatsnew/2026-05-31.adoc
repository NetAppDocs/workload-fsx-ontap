...
=== LIF addresses available in NetApp Workload Factory
We display all storage VM and cluster LIFs directly in the Overview tab for a file system. Direct LIF visibility within the Workload Factory console makes it easier to monitor network access, endpoints, and failover status without using the CLI or CloudShell. To display LIF details, you'll need to associate a link with the file system.
