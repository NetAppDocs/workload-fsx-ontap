...
* You must have credentials with the _______ permission policy in Workload Factory to complete this task (includes for auto capacity we need: fsx::DescribeFileSystems, fsx::UpdateFileSystem, cloudwatch::GetMetricStatistics.).
...
* You must have credentials with the ______ permission policy in Workload Factory to complete this task (includes fsx::DescibeFileSystems). 
warning  field for  nofiication  is enabled only if notification is configured in the account. Ether bluexp or sns.
