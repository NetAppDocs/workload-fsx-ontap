...
[.lead]
Create a replication relationship for an FSx for ONTAP file system in NetApp Workload Factory to avoid data loss in case of an unforeseen disaster.
...
This operation replicates data for one or all source volumes in an on-premises ONTAP system or an FSx for ONTAP file system. 

Replication is supported between two FSx for ONTAP file systems and between an on-premises ONTAP system and an FSx for ONTAP file system.

NOTE: For replication between an on-premises ONTAP system and an FSx for ONTAP file system, only single-volume replication is supported.
