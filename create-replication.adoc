...
For migration use cases, you can replicate storage VM data and configuration settings from an on-premises ONTAP system or a first-generation FSx for ONTAP file system to a second-generation FSx for ONTAP file system. This helps you move to the new file system with minimal downtime and disruption.
...
To migrate storage VM data and configuration settings, you must complete two operations. First <<Create a replication relationship,create a replication relationship>>. Then <<Cut over replication,cut over replication>> to permanently migrate data and configuration settings from the source file system to the target FSx for ONTAP file system.
