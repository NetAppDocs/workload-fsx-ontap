...
Consider the following when planning the cutover operation for migration use cases.

* Storage VM configuration settings: The cutover operation doesn't migrate all storage VM configuration settings to the target storage VM. For example, iSCSI, Broadcast-domain MTU, ONTAP native S3 and S3AP, and DNS settings do not migrate. After cutover, you must manually update these settings on the target storage VM. For more details, refer to the <<Update configuration settings post cutover,Update configuration settings post cutover>> section.

* Clones: Clones get migrated as individual volumes, and therefore, take up double the amount of space in the target FSx for ONTAP file system. Consider deleting these clones before cutover. 

* Volumes with S3 access points: Cut over replication for migration use cases isn't supported for volumes with S3 access points. Consider deleting the S3 access points before cutover.
...
* Update broadcast-domain MTU settings
* Update ONTAP native S3 and S3AP settings
* Update DNS settings
