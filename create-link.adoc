...
+
* GovCloud accounts: 
... Copy the CloudFormation or Terraform code. 
... Open the AWS Management Console, paste and run the code to get the Lambda ARN. 
... In the Workload Factory console, paste the Lambda ARN in the text box. 
... Select *Create*.

* All other accounts: 
... Select CloudFormation or Terraform for the instructions for how to register the link, and follow the instructions. 
+
Note that three permissions are granted to Lambda when you launch the CloudFormation template. Workload factory uses these permissions when using links.
+
[source,json]
