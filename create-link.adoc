...
** The fix: Add permissions after the link is active. Go into AWS console, locate the Lambda link, and edit the attached permission policy.

* Issue: The secret isn't found.
** The fix: Provide the correct secret ARN.

* Issue: The secret isn't in the right format.
** The fix: Go to AWS Secrets Manager and edit the format.
+
The secret should contain the following three parameters:
*** filesystemID = FSx_filesystem_id
*** user = FSx_user
*** password = user_password

* Issue: The secret doesn't contain valid ONTAP credentials for file system authentication. 
** The fix: Provide credentials that can authenticate file systems in AWS Secrets Manager
