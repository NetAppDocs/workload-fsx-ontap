...
Two options are available to create a link - automatically or manually. 

.Before you begin
Consider the following before you begin:
* For creating a link automatically: The CloudFormation code is short-lived. It can only be used for about 6-24 hours. 
* For creating a link manually: The CloudFormation code persists giving you more time to complete the operation. This is helpful when working with different teams like DevOps which may need to first grant the permissions necessary to complete link creation.
