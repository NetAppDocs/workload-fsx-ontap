...
* The link must be able to connect to the following endpoint: \https://docs.netapp.com/us-en/bluexp-setup-admin/reference-networking-saas-console.html. The web-based console contacts this endpoint to interact with the Workload Factory APIs to manage and operate FSx for ONTAP workloads. 
...
. In the Storage tile, select *Go to Storage*. 
. From the Storage menu, select *FSx for ONTAP*. 
. From *FSx for ONTAP*, select the actionsenu of the file system to associate a link to and then select *Associate link*. 
. In the Associate link dialog, select *Create a new link* and select *Continue*.
. On the Create Link page, select CloudFormation or Terraform from the Codebox, and then provide the following: 
.. *Link name*: Enter the name that you want to use for this link. The name must be unique within your account.
.. *AWS Secrets Manager*: Optional. Allows Workload Factory to fetch FSx for ONTAP access credentials from your AWS Secrets Manager. 
+
The link deployment stack automatically adds the following default secret manager ARN regex to the Lambda permission policy: `arn:aws:secretsmanager:<link_deployment_region>:<link_deployment_account_id>:secret:FSxSecret*`. 
