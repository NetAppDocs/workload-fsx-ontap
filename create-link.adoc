...
+
The secret ARN must include the following parameters: 
...
== Troubleshooting issues with AWS Secrets Manager authentication

* Lack of permissions to retrieve the secret
Add permissions after the link is active
Go into AWS console, locate the Lambda link, and edit the attached permission policy.

* Secret isn't found -> respecify the correct secret arm

* Secret isn't in the right format -> go to AWS Secrets Manager and edit the format
The secret should contain these three key valid pairs
	filesystemID = FSx_filesystem_id
	user = FSx_user
	password = user_password

* 	The secret doesn't contain valid ONTAP credentials for file system authentication 
Provide credentials that can authenticate file systems in AWS Secrets Manager
