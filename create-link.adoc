...
* Manually: Creates a link with manual registration using either CloudFormation or Terraform from the Codebox. The CloudFormation code persists giving you more time to complete the operation. This is useful when working with different teams like Security and DevOps that might first need to grant the permissions necessary to complete link creation.
